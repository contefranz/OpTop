  ...
  if (which == "macro") {
    ...
    p <- ggplot2::ggplot(
      ...
      ggplot2::labs(
        ...
        y = "held-out Macro index",
        color = "metric", fill = "metric",
        title = "Held-out Macro discrepancy index",
        subtitle = sprintf("%g%% pointwise intervals", 100 * x$conf)
      )
  } else {
    if (is.null(metric)) {
      metric <- x$metrics[1L]
    ...
    if (is.finite(gt$k_hat)) {
      p <- p + ggplot2::geom_vline(xintercept = gt$k_hat, linetype = 2)
    }
  }
  print(p)
  invisible(p)
