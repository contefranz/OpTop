...
#'
#' @param lda_models A list of LDA models as computed by
...
optimal_topic = function( lda_models, word_proportions,
  ...
  for ( j in seq_along( lda_models ) ) {
    ...
    for ( k in 1L:n_docs ) {
      ...
      prop = word_proportions[ id_doc == k ]
      # subsetting dww according to id_doc
      dwwk = as.matrix( dww[ k ] )

      # casting N x K matrix
      dww_k = matrix( data = dwwk,
                      ncol = ncol( dwwk ),
    ...
    sum_j = chi_out[ , .( chi_sum = sum( chisquare ), cut = sum( row_cut ) ) ]
    temp = data.table( topic = current_k,
                       sum_j,
                       chi_std = sum_j[ 1L, chi_sum ] / sum_j[ 1L, cut ],
                       chi_stdw = chi_out[ , sum( chisquare_mod ) ] / sum_j[ 1L, cut ] )
